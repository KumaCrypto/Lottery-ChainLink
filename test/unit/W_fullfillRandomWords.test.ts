...
import { fixturewithEnteranceAndIncreasedTime } from "./fixture";
		...
		await new Promise<void>(async (resolve, reject) => {
			...
			raffle.once("WinnerPicked", async () => {
				try {
					...
					const endingTimeStamp = await raffle.getLastRaffleTime();
					...
					const shouldBePayedToWinner = await raffle.getWinningFunds(
						recentWinner
					);
            ...
            */
			...
			const winnerBalanceBefore = await signers[2].getBalance();
